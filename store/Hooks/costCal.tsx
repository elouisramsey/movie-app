...
export const totalPriceSelector = createSelector([totalSelector], (item: any) =>
  item.filter((item: any) => item.selected).reduce((total: number, current: any) => total + current.amount, 0)
)
...
export const totalExtraPriceSelector = createSelector(
  ...
  (item: any) =>
    item
      ...
      .reduce((total: number, current: any) => total + current.price * current.quantity, 0)
)
